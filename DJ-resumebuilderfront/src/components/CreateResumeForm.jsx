...
const CreateResumeForm = () => {
  const [title, setTitle] = useState("");  // ✅ Fixed variable name here
  ...
  const handleCreateResume = async (e) => {
    ...
    try {
      ...
      if (response.data?._id) {
        navigate(`/resume/${response.data._id}`); // ✅ Removed trailing space
      }
  ...
  return (
    <div className='w-full max-w-md p-8 bg-white rounded-2xl border border-gray-100 shadow-lg'>
      ...
      <form onSubmit={handleCreateResume}>
        <Input
          value={title}
          onChange={({ target }) => setTitle(target.value)} // ✅ Fixed setter name
          label='Resume Title'
